...
class Admin_products extends CI_Controller {
    ...
    */
    ...
    public function add()
    {
        ...
        if ($this->input->server('REQUEST_METHOD') === 'POST')
        {
            ...
            $this->form_validation->set_rules('fieldid', 'FieldId', 'required');
            $this->form_validation->set_rules('fieldlabel', 'FieldLabel', 'required');
            $this->form_validation->set_rules('fieldtype', 'FieldType', 'required');
            // $this->form_validation->set_rules('fieldEvent', 'fieldEvent', 'required');
            $this->form_validation->set_rules('status', 'FieldStatus', 'required');
            ...
            if ($this->form_validation->run()) {  
                ...
                $data_to_store = array(
                    ...
                    'FieldEvent' => $this->input->post('fieldEvent'),
                    'MasterTableName' => $this->input->post('dropdownadd'),      
    ...
    */
    public function update()
    {
        ...
        if ($this->input->server('REQUEST_METHOD') === 'POST')
        {
            ...
            $this->form_validation->set_rules('fieldlabel', 'FieldLabel', 'required');
            $this->form_validation->set_rules('fieldtype', 'FieldType', 'required');
            $this->form_validation->set_rules('FieldStatus', 'FieldStatus', 'required');
            $this->form_validation->set_error_delimiters('<div class="alert alert-error"><a class="close" data-dismiss="alert">×</a><strong>', '</strong></div>');
            //if the form has passed through the validation
            if ($this->form_validation->run())
            {
                $data_to_store = array(
                    ...
                    'FieldLabel' => $this->input->post('fieldlabel'),
                    'FieldType' => $this->input->post('fieldtype'),          
                    'Status' => $this->input->post('FieldStatus')
                );
                //if the insert has returned true then we show the flash message
    ...
    */
    ...
    function AddField() {
        echo "<input type='text' name='dropdownadd' id='dropdownadd' placeholder='Please enter Mastertable Name'><br/>";
        echo "<div><br/><select name=\"fieldEvent\"><option>Select Field JS Event</option><option>Active</option><option>DeActive</option></select></div><div id=\"field\" style=\"margin:8px 0 0 170px;\"></div>";
    }
